                    ...
                    <td class="application_cell">
                        <g:form action="acceptApplication" controller="goblinOrder">
                            ...
                            <g:submitToRemote update="[success: 'message', failure: 'message']"
                                              onSuccess="hideRow('${applicationNodeId}');"
                                              value="${message(code: 'order.accept.applicant')}"
                                              url="[action: 'acceptApplication', controller: 'goblinOrder']"/>
                        </g:form>
                    </td>
                    <td class="application_cell">
                        <g:form>
                            <input type="hidden" name="application" value="${application.id}">
                            <g:message code="order.deny.reason"/>: <g:textField name="reason"
                                                                                value="// TODO: implement this."/><br>
                            <g:submitToRemote update="[success: 'message', failure: 'message']"
                                              onSuccess="hideRow('${applicationNodeId}');"
                                              value="${message(code: 'order.deny.applicant')}"
                                              url="[action: 'denyApplication', controller: 'goblinOrder']"/>
                        </g:form>
                    </td>
                </tr>
            </g:each>
